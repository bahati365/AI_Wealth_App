import os
from typing import Optional

import streamlit as st
from anthropic import Anthropic

from src.models import PortfolioRecommendation


def get_anthropic_api_key() -> Optional[str]:
    """
    Gets the Claude API key from either:
    1. Local environment variable
    2. Streamlit Cloud secrets
    """
    api_key = os.getenv("ANTHROPIC_API_KEY")

    if not api_key:
        api_key = st.secrets.get("ANTHROPIC_API_KEY", None)

    return api_key


class ExplanationEngine:
    def __init__(self):
        api_key = get_anthropic_api_key()

        if api_key:
            self.client = Anthropic(api_key=api_key)
        else:
            self.client = None

    def generate(self, recommendation: PortfolioRecommendation) -> str:
        """
        Generates an advisor-style explanation.
        Uses Claude if available.
        Falls back to rule-based explanation if Claude fails.
        """
        if self.client is None:
            return self._fallback_explanation(recommendation)

        prompt = self._build_prompt(recommendation)

        try:
            response = self.client.messages.create(
                model="claude-haiku-4-5",
                max_tokens=350,
                messages=[
                    {"role": "user", "content": prompt}
                ],
            )

            return response.content[0].text

        except Exception:
            return self._fallback_explanation(recommendation)

    def _build_prompt(self, recommendation: PortfolioRecommendation) -> str:
        profile = recommendation.profile
        allocation = recommendation.allocation

        return f"""
You are an educational wealth advisor simulator.

Explain this portfolio recommendation in plain English.

Client profile:
- Age range: {profile.age_range}
- Investment purpose: {profile.investment_purpose}
- Investment style: {profile.goal}
- Risk tolerance: {profile.risk_tolerance}
- Time horizon: {profile.time_horizon}

Portfolio allocation:
- Equities: {allocation["Equities"]}%
- Bonds: {allocation["Bonds"]}%
- Cash: {allocation["Cash"]}%

Rules:
- Do not say returns are guaranteed.
- Do not give licensed financial advice.
- Explain the tradeoffs clearly.
- Keep it short, practical, and beginner-friendly.
- Mention that this is educational only.
"""

    def _fallback_explanation(self, recommendation: PortfolioRecommendation) -> str:
        profile = recommendation.profile
        allocation = recommendation.allocation

        return f"""
Based on your profile, this portfolio is designed for someone investing for **{profile.investment_purpose.lower()}**
with a **{profile.time_horizon.lower()}** time horizon.

Your selected investment style is **{profile.goal.lower()}**, and your risk tolerance is **{profile.risk_tolerance.lower()}**.

Recommended allocation:

- **Equities:** {allocation["Equities"]}%
- **Bonds:** {allocation["Bonds"]}%
- **Cash:** {allocation["Cash"]}%

This mix balances growth potential, stability, and liquidity based on your selected profile.
Equities support long-term growth, bonds help reduce volatility, and cash provides flexibility for shorter-term needs.

This is an educational simulation only and should not be treated as financial advice.
"""